...
```{r, message=FALSE}
crime <- read_csv("https://www.kaggle.com/sandeep04201988/housing-price-index-using-crime-rate-data/version/1")
head(crime)
```
...
Now you need to load our first data file.  When we load it, we assign it to a new variable we're caloing `my_data`. Note, the bitly link is just a short cut to where the datafile is in my GitHub account (https://raw.githubusercontent.com/ajstewartlang/Keele_Sept_2019/master/Afternoon/ANOVA_data1.csv).
...
Open our third datafile and map it onto the variable `my_data3`.
